...
<table>
  <tr>
    <td><a href="hp0201.php?os=7983&doc=7">鹿児島県霧島市国分中央　土地付建物</a></td>
...
def test_nta_parse_rows():
    ...
    assert [ls.listing_id for ls in items] == ["os7983", "os8001"]
    a = items[0]
    ...
    assert items[1].address.startswith("宮城県仙台市")
...
<ul>
  <li><a href="/ab06/kensei/nyusatu/zaisan/bukenholder/kyuutaiseiryou.html">
    物件明細（旧大成寮）</a></li>
  <li><a href="/ab06/kensei/nyusatu/zaisan/bukenholder/ijuuin.html">
    物件明細（旧伊集院高校教職員住宅）</a></li>
...
def test_pref_parse_index():
    ...
    assert len(items) == 2   # 物件明細だけ。案内リンクは拾わない
    a = items[0]
